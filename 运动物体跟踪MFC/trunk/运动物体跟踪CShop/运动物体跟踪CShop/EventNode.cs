...
namespace 运动物体跟踪CShop
{
    ...
    {
        static Bgr[] sampleColor = new Bgr[5] {new Bgr(Color.Red), new Bgr(Color.Blue), new Bgr(Color.Black), new Bgr(Color.Brown), new Bgr(Color.Green) };
        ...
        //判断两个矩形框是否为同一个对象
        static bool isTheSame(Rectangle r1, Rectangle r2)
        {
        ...
        //插入一个新事件
        static EventNode insertEventNode(ref List<EventNode> eventList, Rectangle r, int frameNum)
        {
        ...
        //删除整个事件列表
        static void deleteEventList(ref List<EventNode> eventList)
        {
            eventList.Clear();
        }

        //遍历链表,更新跟踪标记
        static void bianliEventList(ref List<EventNode> eventList, int endFrame)
        {
        ...
        //过滤列表中短的事件
        static void eventFilter(ref List<EventNode> eventList)
        {
            for (int i = 0; i < eventList.Count; i++)
            {
        ...
        //搜索链表,检查有没有相同事件
        static EventNode searchEventList(ref List<EventNode> eventList, Rectangle r2)
        {
            for (int i = 0; i < eventList.Count; i++)
            {
                if (eventList[i].endFrame != -1)
